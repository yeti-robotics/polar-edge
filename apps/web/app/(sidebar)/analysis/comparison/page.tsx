...
export default async function AnalysisComparisonPage({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
	...
	const coralTotals = await db.select({
	...
	}).from(standForm).where(eq(standForm.teamNumber, parseInt(teamNumber as string)));
