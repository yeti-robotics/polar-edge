      ...
      <div className="mt-1.5 space-y-0.5">
        {payload.map((entry) => (
          <div key={entry.name} className="flex items-center gap-2 text-sm">
            ...
            <span className="tabular-nums font-medium">{entry.value}s</span>
          </div>
        ))}
      </div>
      <p className="mt-1 text-xs text-muted-foreground tabular-nums">
        Total: {(point.auto + point.teleop).toFixed(1)}s
      </p>
    </div>
  );
}

export function ShootingTimeByMatchChart({ data }: { data: CycleTimeseriesPoint[] }) {
  if (data.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Shooting Time by Match</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  const chartData: ChartPoint[] = data.map((point, i) => ({
    index: i + 1,
    auto: point.autoShootingTime,
    teleop: point.teleopShootingTime,
      ...
      <CardContent>
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={chartData} margin={{ top: 8, right: 12, bottom: 20, left: -16 }}>
            ...
            />
            <Bar dataKey="auto" name="Auto" stackId="time" fill="#f59e0b" radius={[0, 0, 0, 0]} />
            <Bar
              dataKey="teleop"
              name="Teleop"
              stackId="time"
