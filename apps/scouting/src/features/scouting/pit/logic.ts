...
export async function insertPitForm(data: typeof pitForm.$inferInsert, photoKeys: string[]) {
  const [insertedForm] = await db.insert(pitForm).values(data).returning({ id: pitForm.id });
