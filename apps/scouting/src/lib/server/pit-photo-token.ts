...
function getSecret(): string {
  const secret = process.env.PIT_PHOTO_TOKEN_SECRET ?? process.env.AUTH_SECRET;
  if (!secret) {
    throw new Error("Set PIT_PHOTO_TOKEN_SECRET or AUTH_SECRET for pit photo view tokens.");
  }
  return secret;
}
 ...
 */
export function verifyPitPhotoViewToken(token: string): { storageKey: string } | null {
  try {
